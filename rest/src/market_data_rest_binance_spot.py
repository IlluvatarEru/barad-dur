...
from core.src.markets import BINANCE
...
class MarketDataRestApiBinanceSpot(MarketDataRestApi):
    """
    A base class for Binance Spot Api
    """
    ...
    def __init__(self,
        ...
        :param private_key:
        """
        super().__init__(BINANCE, SPOT, public_path, private_path, public_key, private_key)
    ...
    def format_sym_back(self, sym):
        ...
        crypto = self.format_crypto_back(sym[:3])
        ccy = crypto + fiat
        return ccy

    def format_crypto_back(self, crypto):
        """
        Format the crypto back from the exchange format to our standard format
